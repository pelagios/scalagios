...
class PlacePatchCollector extends ResourceCollector {
  ...
  lazy val patches: Iterable[PlacePatch] = {
    ...
    resourcesOfType(LAWD.Place).map(resource => {
      ...
      toPlacePatch(resource, names, location, geometry, images)
    })
  } 
  
  private def toPlacePatch(resource: Resource, names: Seq[PlainLiteral], location: Option[Coordinate],
      geometry: Option[Geometry], depictions: Seq[Image]) = PlacePatch(
